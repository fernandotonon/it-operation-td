...
Item {
    ...
    // Desktop / dojo sound objects live in a Loader that stays inactive on WebAssembly: a Qt audio sink
    // opened there stalls the page.
    readonly property var sounds: desktopSounds.item ? desktopSounds.item.sounds : ({})
    Loader {
        id: desktopSounds
        active: Qt.platform.os !== "wasm"
        sourceComponent: Component {
            Item {
                readonly property var sounds: ({ shot: snd_shot, burst: snd_burst, hit: snd_hit, death: snd_death, escape: snd_escape, place: snd_place, sell: snd_sell,
                                                 upgrade: snd_upgrade, wave: snd_wave, win: snd_win, lose: snd_lose, click: snd_click, reboot: snd_reboot, alarm: snd_alarm, reveal: snd_reveal })
